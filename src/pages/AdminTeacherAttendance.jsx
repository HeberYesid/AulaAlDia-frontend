...
import { listTeacherAttendanceLogs } from '../hooks/useTeacherAttendance'
...
export default function AdminTeacherAttendance() {
  ...
  async function loadLogs(nextFilters = filters) {
    ...
    setError('')
    try {
      const data = await listTeacherAttendanceLogs(buildQueryParams(nextFilters))
      ...
      setError(
        getApiErrorMessage(err, {
          action: 'cargar los registros de asistencia docente',
          fallback:
            'No se pudieron cargar los registros de asistencia docente. Verificá los filtros y reintentá.',
        })
      )
    } finally {
      setLoading(false)
  ...
  return (
    <section className="fade-in" style={{ display: 'grid', gap: 'var(--space-lg)' }}>
      <div className="card">
        <h1 style={{ marginBottom: 'var(--space-sm)' }}>Asistencia Docente</h1>
        <p style={{ color: 'var(--text-secondary)' }}>
          Monitoreá entradas y salidas docentes por fecha, estado y docente.
        </p>
      </div>
      ...
      <form className="card" onSubmit={handleSubmit}>
        <div className="grid cols-4">
          <div className="form-group">
            <label htmlFor="attendance-teacher-id">Docente (ID)</label>
            <input
              id="attendance-teacher-id"
              type="number"
              min="1"
              value={filters.teacher_id}
              onChange={(event) => handleFilterChange('teacher_id', event.target.value)}
            />
          </div>

          <div className="form-group">
            <label htmlFor="attendance-date-from">Desde</label>
            <input
          ...
          <div className="form-group">
            <label htmlFor="attendance-date-to">Hasta</label>
            <input
        ...
        <div className="card">
          ...
          {loading ? (
            <div className="loading">
              <div className="spinner" role="status" aria-label="Cargando registros de asistencia docente"></div>
              <span aria-hidden="true">Cargando registros…</span>
            </div>
          ) : logs.length === 0 ? (
            <p style={{ color: 'var(--text-secondary)' }}>Sin registros de asistencia docente.</p>
          ) : (
            <div className="data-table">
              <table className="table mobile-card-view">
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.id}>
                      <td data-label="Docente">{log.teacher?.full_name || log.teacher?.email}</td>
        ...
        <div className="card">
          ...
          {!selectedLog ? (
            ...
            <div style={{ display: 'grid', gap: 'var(--space-sm)' }}>
              <p>
                <strong>Docente:</strong> {selectedLog.teacher?.full_name || selectedLog.teacher?.email}
              </p>
              <p>
                <strong>Email:</strong> {selectedLog.teacher?.email}
              </p>
              <p>
